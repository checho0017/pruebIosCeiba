...
extension ApiClient: UsersApiProtocol
{
    func getUsers() -> AnyPublisher<Alamofire.DataResponse<[UserApi], ApiError>, Never> {
        guard let url = URL(string: "\(apiEndpoints.urlBase)\(apiEndpoints.getUsers)")
                else
        {
            return emptyPublisher(error: CustomError(code: 555, message: Constants.apiUrlNotFound))
        }
    ...
    func getUsersPosts(id: Int) -> AnyPublisher<Alamofire.DataResponse<[UserPublication], ApiError>, Never> {
        guard let url = URL(string: "\(apiEndpoints.urlBase)\(apiEndpoints.getPublicationsByIdUser)\(String(id))")
                else
        {
            return emptyPublisher(error: CustomError(code: 555, message: Constants.apiUrlNotFound))
        }
